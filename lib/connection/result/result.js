...
function Result(options) {
  ...
  // Debug logging when rowset is unexpectedly missing
  if (data.rowset == null && data.returned > 0) {
    Logger().error(
      `Missing rowset in response for queryId: ${data.queryId}. ` +
        `Expected ${data.returned} rows but rowset is ${data.rowset === null ? 'null' : 'undefined'}. ` +
        `queryResultFormat: ${data.queryResultFormat || 'not specified'}, ` +
 ...
 * @param statement
 * @param services
 * @param queryResultFormat - The format of query results ('json' expected, 'arrow' indicates a problem)
 * @param queryId - The query ID for debugging purposes
 *
 * @returns {Chunk}
 */
function createChunks(
  ...
  // Guard against unexpected arrow format or missing rowset
  if (queryResultFormat && queryResultFormat !== 'json') {
    Logger().error(
      `Unexpected queryResultFormat: ${queryResultFormat} for queryId: ${queryId}. ` +
        `Expected 'json'. rowset is ${rowset == null ? 'null/undefined' : 'present'}, ` +
        `chunks count: ${chunkCfgs ? chunkCfgs.length : 0}`,
    );
  }

  if (rowset == null) {
    Logger().error(
      `rowset is ${rowset === null ? 'null' : 'undefined'} for queryId: ${queryId}. ` +
        `queryResultFormat: ${queryResultFormat || 'not specified'}, ` +
        `chunks count: ${chunkCfgs ? chunkCfgs.length : 0}. ` +
        `This may indicate the server returned arrow format data while claiming json format.`,
    );
    rowset = [];
  }
